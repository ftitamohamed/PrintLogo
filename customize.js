...
// Function to generate images from local storage data
/* async function generateImagesFromLocalStorage() {
    const storages = ['Front', 'Back', 'Left', 'Right'];
    const results = {}; // Store generated image URLs
    ...
    for (const storageName of storages) {
        ...
        if (jsonData) {
            ...
            tempCanvasElement.width = canvasData.objects[0]?.width || 400;
            tempCanvasElement.height = canvasData.objects[0]?.height || 400;
            tempCanvasElement.style.position = 'absolute';
            tempCanvasElement.style.left = '-9999px'; // Hide it offscreen
            document.body.appendChild(tempCanvasElement); // Append it to DOM

            // Initialize fabric.Canvas with the created <canvas> element
            const tempCanvas = new fabric.Canvas(tempCanvasElement);

            // Load the saved JSON data onto the canvas
            ...
            // Wait for the canvas to be rendered, then capture the image using html2canvas
            const canvasImage = await html2canvas(tempCanvasElement, {
                useCORS: true, // Handle cross-origin images
                backgroundColor: null, // Set transparent background
            }).then((canvas) => canvas.toDataURL('image/png')); // Convert to data URL

            // Store the image result
            console.log(canvasImage);
            results[storageName] = canvasImage;

            // Ensure that the canvas element is removed after rendering and image capture
            setTimeout(() => {
                document.body.removeChild(tempCanvasElement); // Safely remove canvas after async operations
            }, 0); // Using setTimeout to delay the removal just enough
        }
    }

    return results; // Return the object containing all images
}



// Usage example
function showPreviewWithImages() {
    generateImagesFromLocalStorage().then((images) => {
        // Create a centered preview div
        ...
        closeButton.addEventListener('click', () => {
            // Remove the preview div on close button click
        ...
        Object.entries(images).forEach(([key, dataUrl]) => {
            ...
            imgElement.style.width = '150px'; // Adjust image size as needed
            ...
            label.innerText = key; // Add a label below each image
        ...
        // Append the preview div to the document body
        document.body.appendChild(previewDiv);
    });
}

// Example usage: Call the function to show the preview
const previewButton = document.getElementById('preview');

previewButton.style.cursor = 'pointer'; */

/* previewButton.addEventListener('click', showPreviewWithImages); */
  ...
  function saveToLocalStorage() {
    ...
    html2canvas(node, { useCORS: true }).then(function (canvas) {
        ...
        const newOrder = {
            ...
            back_image: selectedColor.back_image || "",
            right_side_image: selectedColor.right_side_image || "",
            left_side_image: selectedColor.left_side_image || "",
            logos: ""
        };
...
buyNow.addEventListener('click',()=>{
    console.log('this is the orders length : 5',);
        saveToLocalStorage()
